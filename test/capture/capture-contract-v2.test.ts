...
import { CAPTURE_SCHEMA, parseCaptureEvent, SUPPORTED_OPENCODE_VERSION, type CaptureEventV1 } from "../../src/capture/contract";
...
describe("capture contract v2 hardening", () => {
  test("uses the schema 11 capture contract", () => {
    expect(String(CAPTURE_SCHEMA)).toBe("agz-memory.capture/2");
  });

  test("AGZ-006 separates capture identity tuples that collide on NUL delimiters", () => {
    const pairs = [
      [
        { kind: "user", bindingKey: "binding", sessionID: "session\0message", messageID: "tail" },
        { kind: "user", bindingKey: "binding", sessionID: "session", messageID: "message\0tail" },
      ],
      [
        {
          kind: "assistant",
      ...
      [
        { kind: "summary", bindingKey: "binding\0session", sessionID: "id", checkpointMessageID: "checkpoint" },
        { kind: "summary", bindingKey: "binding", sessionID: "session\0id", checkpointMessageID: "checkpoint" },
      ],
    ] as const;

    for (const [left, right] of pairs) {
      expect(captureIdempotencyKey(left)).not.toBe(captureIdempotencyKey(right));
    }
  });
  ...
  test("AGZ-007 rejects a reused idempotency key with different source or payload", () => {
    const directory = mkdtempSync(join(tmpdir(), "agz-memory-capture-conflict-"));
    const opened = openMemoryDatabase(join(directory, "memory.sqlite"));
    try {
      const memory = new MemoryStore(opened.db);
      const projectID = memory.createProject("Capture Conflict").project!.projectID;
      const capture = new CaptureStore(opened.db);
      ...
      const first = captureEvent(projectID, binding.bindingKey, "first payload", 1);
      ...
      expect(() => capture.ingest(sourceConflict, "shadow")).toThrow("idempotency_conflict");
      expect(() => capture.ingest(payloadConflict, "shadow")).toThrow("idempotency_conflict");
      expect(
        (opened.db.query("SELECT COUNT(*) AS count FROM capture_events").get() as { count: number }).count,
      ).toBe(1);
      expect(
        (opened.db.query("SELECT payload_json FROM capture_events").get() as { payload_json: string }).payload_json,
  ...
  test("AGZ-019 enforces kind-specific source identity fields", () => {
    ...
    expect(() => parseCaptureEvent(withSource(user, { toolCallID: "tool-1" }))).toThrow();

    const assistant = validEvent("assistant-candidate");
    expect(() => parseCaptureEvent(withoutSource(assistant, "ordinal"))).toThrow();
    expect(() => parseCaptureEvent(withSource(assistant, { toolCallID: "tool-1" }))).toThrow();

    const summary = validEvent("session-summary");
    expect(() => parseCaptureEvent(withoutSource(summary, "messageID"))).toThrow();
    expect(() => parseCaptureEvent(withSource(summary, { ordinal: 0 }))).toThrow();

    const tool = validEvent("tool-signal");
    expect(() => parseCaptureEvent(withoutSource(tool, "messageID"))).toThrow();
    expect(() => parseCaptureEvent(withoutSource(tool, "toolCallID"))).toThrow();
    ...
    try {
      ...
      expect(() =>
        opened.db
          ...
          .run("cross-project-checkpoint", bindingB.bindingKey, projectA, 1, 1, 1),
          ...
          .query(
            `INSERT INTO capture_events
              (idempotency_key, contract, project_id, binding_key, event_kind,
               source_session_id, redaction_version, state, created_at, updated_at)
             VALUES (?, ?, ?, ?, 'user-candidate', ?, 'redaction/1', 'pending', ?, ?)`,
          )
          .run("c".repeat(64), CAPTURE_SCHEMA, projectA, bindingB.bindingKey, "cross-project-event", 1, 1),
      ).toThrow(/foreign key/i);
        ...
        opened.db
          .query(
            `INSERT INTO index_outbox
              (backend, operation, project_id, note_id, revision, available_at, created_at)
             VALUES (?, 'upsert-note', ?, ?, 1, ?, ?)`,
          )
          .run("fake@1", projectA, noteB, 1, 1),
      ).toThrow(/foreign key/i);
  ...
  test("AGZ-023 permits one session ID in two distinct bindings", () => {
    const directory = mkdtempSync(join(tmpdir(), "agz-memory-capture-session-binding-"));
    const opened = openMemoryDatabase(join(directory, "memory.sqlite"));
    try {
      const memory = new MemoryStore(opened.db);
      const projectA = memory.createProject("Session Binding A").project!.projectID;
      const projectB = memory.createProject("Session Binding B").project!.projectID;
      const capture = new CaptureStore(opened.db);
      const bindingA = capture.bindProject({
        memoryProjectID: projectA,
      ...
      expect(() => capture.checkpoint("shared-session", bindingA.bindingKey, projectA)).not.toThrow();
      expect(() => capture.checkpoint("shared-session", bindingB.bindingKey, projectB)).not.toThrow();
      expect(
        opened.db
          .query(
        ...
        { session_id: "shared-session", binding_key: bindingA.bindingKey, project_id: projectA },
        { session_id: "shared-session", binding_key: bindingB.bindingKey, project_id: projectB },
...
function validEvent(kind: CaptureEventV1["kind"]): CaptureEventV1 {
  ...
  if (kind === "assistant-candidate") {
    return {
      ...
      source: source({ sessionID: "session-1", messageID: "assistant-1", ordinal: 0 }),
  ...
  return {
    ...
    source: source({ sessionID: "session-1", messageID: "assistant-1", toolCallID: "tool-1" }),
...
function candidate(evidence: "explicit-user" | "verified-outcome" | "session-summary") {
  return {
    kind: "fact" as const,
...
function withSource(event: CaptureEventV1, fields: Record<string, unknown>): unknown {
